...
_TLDR_CACHE_MAX_AGE_SECS = 600  # Re-warm if call graph is older than 10 minutes


def _maybe_warm_tldr(workspace_path: Path, job_id: str) -> None:
    """Run `tldr warm` if the call graph cache is missing or older than 10 minutes.

    Runs unconditionally — does not require a git remote. This ensures locally-generated
    jobs get a fresh call graph for impact discovery and symbol map injection.
    """
    import time
    call_graph = workspace_path / ".tldr" / "cache" / "call_graph.json"
    try:
        age = time.time() - call_graph.stat().st_mtime if call_graph.exists() else float("inf")
    except OSError:
        age = float("inf")

    if age < _TLDR_CACHE_MAX_AGE_SECS:
        logger.debug("tldr cache fresh (%.0fs old), skipping warm (job %s)", age, job_id)
        return

    try:
        from llamaindex_crew.tools.tldr_tools import run_tldr
        run_tldr(["warm", str(workspace_path), "--lang", "all"])
        logger.info("tldr warm completed (cache was %.0fs old, job %s)", age, job_id)
    except Exception as e:
        logger.warning("tldr warm failed (non-fatal): %s", e)


def _pull_and_reindex(workspace_path: Path, job_db: Any, job_id: str) -> None:
    """Pull latest changes from the git remote (if any).

    Best-effort: any failure here is logged and refinement proceeds with whatever is present.
    tldr warm is handled separately by _maybe_warm_tldr so local jobs also benefit.
    """
    workspace_path = Path(workspace_path)
    if not (workspace_path / ".git").exists():
        return
...
def _workspace_has_changes(workspace_path: Path) -> bool:
    ...
    try:
        ...
        real = [p for p in changed + untracked if _is_real_change(p)]
        return bool(real)
    except Exception as e:
...
# Directories to skip during recursive scanning — one shared definition,
# see llamaindex_crew/utils/vendor_paths.
...
def _run_impact_refinement(
    ...
    """Impact-aware refinement: process each impacted file sequentially."""
    ...
    progress_callback("refining", 15, f"Impact scope: {', '.join(allowed)}")

    files_modified: List[str] = []
    total = len(allowed)

    for idx, fp in enumerate(allowed):
        pct = 20 + int(60 * idx / max(total, 1))
        progress_callback("refining", pct, f"Refining {fp} ({idx + 1}/{total})...")

        # Snapshot BEFORE running the agent so the diff baseline is clean.
        # Read current content for display in prompt AND as before-state for change detection.
        _git_snapshot(workspace_path)
        initial_content = _load_file_content(workspace_path, fp)
        content_before = initial_content or ""
        ...
        for attempt in range(1, MAX_AGENT_RETRIES + 1):
            try:
                ...
                retry_prompt = prompt
                if attempt > 1:
                    logger.warning("Impact refinement retry %d/%d for %s", attempt, MAX_AGENT_RETRIES, fp)
                    retry_prompt = (
                        f"{prompt}\n\n"
                ...
                logger.exception("Impact refinement failed for %s: %s", fp, e)
                break

            # Detect change by comparing file content before/after — avoids false
            # negatives when _workspace_has_changes sees a clean tree because a
            # subsequent loop iteration already snapshotted the changes.
            content_after = _load_file_content(workspace_path, fp) or ""
            file_changed = content_after != content_before
            if not file_changed:
                # Fall back to git dirty check (handles new-file creation)
                file_changed = _workspace_has_changes(workspace_path)
            if file_changed:
                logger.info("Impact refinement wrote changes for %s (attempt %d)", fp, attempt)
                break
            logger.warning("Impact refinement no writes for %s (attempt %d/%d)", fp, attempt, MAX_AGENT_RETRIES)
    ...
    if not files_modified:
        return _fail_refinement(
            ...
            "Impact refinement completed but no files were changed. "
            "Try rephrasing your request or being more specific.", previous_status,
        )
